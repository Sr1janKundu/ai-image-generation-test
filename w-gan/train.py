...
for epoch in range(NUM_EPOCHS):
    ...
    for batch_idx, (real, _) in tqdm(enumerate(loader), total=len(loader), desc=f"Epoch {epoch + 1}"):
        ...
        for _ in range(CRITIC_ITERATIONS):
            ...
            for p in critic.parameters():       # grad clipping
                p.data.clamp_(-WEIGHT_CLIP, WEIGHT_CLIP)       # _ in clamp_ ensures done inplace
    ...
    if (epoch + 1) % 50 == 0:
        ...
        torch.save(checkpoint, f'models/wgan_checkpoint_epoch_{epoch + 1}.pth')
        print(f"\nModel saved at models/wgan_checkpoint_epoch_{epoch + 1}.pth")
...
torch.save(final_checkpoint, 'models/wgan_checkpoint_final.pth')
print("\n\nFinal model saved at models/wgan_checkpoint_final.pth")
...
""" Usage
checkpoint = torch.load('models/wgan_checkpoint_final.pth')
