"""
Some portion of the official implementation (https://github.com/hanzhanggit/StackGAN-Pytorch/tree/master)
deviates from the paper (https://arxiv.org/pdf/1612.03242), as listed below:

1. The stated 1x1 convolution for jointly learning image-text alignment is not used in the discriminator stages
2. There are some tricks in how losses are calculated in both stages (checking image-text alignment in both stages,
   but only checking "real"ness in generated stage 2 image)
3. The discriminator in both stages does not take the text embedding, rather it takes the mean vector calculated in
   the ConditionalAugmentation class that is returned by the generator of that stage.

This script changes the discriminator classes, and the required functions for loss calculation in the utils.py file
are added accordingly as well.
"""
...
class D_GET_LOGITS(nn.Module):
    """
    Taken from original code, but removed sigmoid layer to work with fp16 training while using BCEWithLogitLoss
    and added processing for text embeddings
    """
    ...
    def forward(self, h_code, c_code=None):
        ...
        if self.bcondition and c_code is not None:
            c_code = c_code.view(c_code.shape[0], self.ef_dim, 1, 1)        # Reshaped to the desired shape [batch, 128] --> [batch, 128, 1, 1]
            c_code = c_code.expand(c_code.shape[0], self.ef_dim, 4, 4)      # [batch, 128, 1, 1] --> [batch, 128, 4, 4]

            # state size (ngf+egf) x 4 x 4
            h_c_code = torch.cat((h_code, c_code), 1)
        else:
            h_c_code = h_code
        # print(f"h_c_code shape: {h_c_code.size()}")
...
class DiscriminatorStage1(nn.Module):
    ...
    def __init__(self):
        ...
        self.get_cond_logits = D_GET_LOGITS(self.df_dim, self.ca_shape, bcondition=True)
...
class DiscriminatorStage2(nn.Module):
    ...
    def __init__(self):
        ...
        self.get_cond_logits = D_GET_LOGITS(self.df_dim, self.ca_shape, bcondition=True)
        self.get_uncond_logits = D_GET_LOGITS(self.df_dim, self.ca_shape, bcondition=False)
...
def test():
    ...
    print(f"Stage 1 Img device: {img_stage1.device}, MU stage1 shape: {mu_1.size()}, MU device: {mu_1.device}, SD device: {sig_1.device}")
    ...
    stage1_img_emb = dis1(img_stage1)
    print(f"Stage 1 emage embedding shape by stage 1 discriminator: {stage1_img_emb.size()}")   # Expected: [2, 512, 4, 4]
    ...
    print(f"Stage 2 Img 1 device: {img_stage1_from_gen2.device}, Stage 2 Img 2 device: {img_stage2.device}, MU stage2 shape: {mu_2.size()}, MU device: {mu_2.device}, SD device: {sig_2.device}")
    ...
    stage2_img_emb = dis2(img_stage2)
    print(f"Stage 2 emage embedding shape by stage 2 discriminator: {stage2_img_emb.size()}")   # Expected: [2, 512, 4, 4]
