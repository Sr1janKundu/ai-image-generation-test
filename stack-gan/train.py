...
def train(
    ...
    if train_stage_2 and epoch > config.hyperparameters['train_stage1_for']:
        ...
        for batch_idx, (img, cap_emb, captions) in enumerate(loop_s2):
            ...
            caps_emb = cap_emb.to(config.device)
            ...
            with torch.amp.autocast('cuda'):
                ...
                # for real images
                dis_s2_real = dis_s2(img, caps_emb)

                # for fake images
                dis_s2_fake_g1 = dis_s1(img, torch.roll(caps_emb, 1, 0))
                dis_s2_fake_g2 = dis_s1(img_stage2.detach(),
                                        caps_emb)  # .detach() in order to reuse dis_s1_real in calculating stage1 generator loss
            ...
            with torch.amp.autocast('cuda'):
                dis_s2_fake = dis_s1(img_stage2, caps_emb)
                # calculate loss
